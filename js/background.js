...
/**
    请求头像
**/
// `https://api.twitter.com/1.1/users/search.json?q=${handle}`,
function getTwitterAvatar(handle, guest_token) {
    return fetch(
...
function getTwitterAvatarToken() {
    return fetch(
...
async function loopSummary(firstItem) {
    ...
    if(firstItem) {
        ...
        if(queneItem.twitter_handle) {
            let twitterHandleTokenRes = {}
            let twitterHandleRes = {profile_image_url:''}
            try {
                twitterHandleTokenRes = await getTwitterAvatarToken()
            } catch (error) {}
            try {
                twitterHandleRes = await getTwitterAvatar(queneItem.twitter_handle, twitterHandleTokenRes.guest_token);
            } catch (error) {}
            result.twitter_avatar = twitterHandleRes.profile_image_url;
        }
        let profileProjectRes = {};
        try {
...
function summaryQuene(msg, sendResponse) {
    ...
    let queneItem = {
        ...
        tid: msg.data.tid,
        twitter_handle: msg.data.twitter_handle,
