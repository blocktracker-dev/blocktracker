 ...
 * 插入sponsored 模块
*/
function insertSponsored(partent) {
  const sponsoredDom = createElement('div',
    {'class': 'g-sponsored'},
    `<span>Sponsored by</span><img src="${IMG_URL['maodao']}"><span class="m-mao-dao">MaoDao</span>`
  );
  partent.appendChild(sponsoredDom)
}

/**
 * maodao string
*/
function sponsoredString() {
  return `<div class="g-sponsored">
    <span>Sponsored by</span><span class="m-mao-dao">MaoDao</span><img src="${IMG_URL['maodao']}">
  </div>`
}

document.addEventListener('click', e => {
  const target = e.target
  if (target.getAttribute('class') == 'g-sponsored' || target.parentNode.getAttribute('class') == 'g-sponsored') {
